...
function byteback.dummy.complete.DoubleSelectionSort.is_minimum#double?#int#int#int#(~heap : Store, $a : Reference, $i : int, $j : int, $m : int) returns (bool) { (forall $k : int :: ~implies((~int.lte($i, $k) && ~int.lt($k, $j)), ~real.gte((~unbox(~heap.read(~heap, $a, ~element($k))) : real), (~unbox(~heap.read(~heap, $a, ~element($m))) : real)))) }
...
function byteback.dummy.complete.DoubleSelectionSort.sorted#double?#int#int#(~heap : Store, $a : Reference, $i : int, $j : int) returns (bool) { (forall $k : int :: ~implies((~int.lt($i, $k) && ~int.lt($k, $j)), ~real.lte((~unbox(~heap.read(~heap, $a, ~element(($k - 1)))) : real), (~unbox(~heap.read(~heap, $a, ~element($k))) : real)))) }

function byteback.dummy.complete.DoubleSelectionSort.partitioned#double?#int#(~heap : Store, $a : Reference, $c : int) returns (bool) { (forall $k : int :: (forall $l : int :: ~implies((((~int.lte(0, $k) && ~int.lt($k, $c)) && ~int.lte($c, $l)) && ~int.lt($l, ~lengthof($a))), ~real.lte((~unbox(~heap.read(~heap, $a, ~element($k))) : real), (~unbox(~heap.read(~heap, $a, ~element($l))) : real))))) }
...
label2:
  ...
  call $m := byteback.dummy.complete.DoubleSelectionSort.minimum#double?#int#($a, $c);
  ~heap := ~heap.update(~heap, $a, ~element($m), ~box((~unbox(~heap.read(~heap, $a, ~element($c))) : real)));
