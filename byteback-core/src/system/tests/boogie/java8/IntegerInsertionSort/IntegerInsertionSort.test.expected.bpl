...
const unique $byteback.dummy.complete.IntegerInsertionSort : Type;
...
function byteback.dummy.complete.IntegerInsertionSort.sorted#int?#int#int#(~heap : Store, $a : Reference, $i : int, $j : int) returns (bool) { (forall $k : int :: ~implies((~int.lt($i, $k) && ~int.lt($k, $j)), ~int.lte((~unbox(~heap.read(~heap, $a, ~element(($k - 1)))) : int), (~unbox(~heap.read(~heap, $a, ~element($k))) : int)))) }
