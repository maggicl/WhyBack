...
function byteback.dummy.complete.IntegerSelectionSort.is_minimum#int?#int#int#int#(~heap : Store, $a : Reference, $i : int, $j : int, $m : int) returns (bool) { (forall $k : int :: ~implies((~int.lte($i, $k) && ~int.lt($k, $j)), ~int.gte((~unbox(~heap.read(~heap, $a, ~element($k))) : int), (~unbox(~heap.read(~heap, $a, ~element($m))) : int)))) }
...
function byteback.dummy.complete.IntegerSelectionSort.sorted#int?#int#int#(~heap : Store, $a : Reference, $i : int, $j : int) returns (bool) { (forall $k : int :: ~implies((~int.lt($i, $k) && ~int.lt($k, $j)), ~int.lte((~unbox(~heap.read(~heap, $a, ~element(($k - 1)))) : int), (~unbox(~heap.read(~heap, $a, ~element($k))) : int)))) }

function byteback.dummy.complete.IntegerSelectionSort.partitioned#int?#int#(~heap : Store, $a : Reference, $c : int) returns (bool) { (forall $k : int :: (forall $l : int :: ~implies((((~int.lte(0, $k) && ~int.lt($k, $c)) && ~int.lte($c, $l)) && ~int.lt($l, ~lengthof($a))), ~int.lte((~unbox(~heap.read(~heap, $a, ~element($k))) : int), (~unbox(~heap.read(~heap, $a, ~element($l))) : int))))) }
...
label2:
  ...
  call $m := byteback.dummy.complete.IntegerSelectionSort.minimum#int?#int#($a, $c);
  ~heap := ~heap.update(~heap, $a, ~element($m), ~box((~unbox(~heap.read(~heap, $a, ~element($c))) : int)));
