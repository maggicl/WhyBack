...
function byteback.dummy.complete.GCD.gcd_recursive#int#int#(~heap : Store, $this : Reference, $a : int, $b : int) returns (int) { if ~eq($a, $b) then $a else if ~int.gt($a, $b) then byteback.dummy.complete.GCD.gcd_recursive#int#int#(~heap, $this, ($a - $b), $b) else byteback.dummy.complete.GCD.gcd_recursive#int#int#(~heap, $this, $a, ($b - $a)) }

procedure byteback.dummy.complete.GCD.apply#int#int#(?this : Reference where (~typeof(~heap, ?this) == $byteback.dummy.complete.GCD), ?a : int, ?b : int) returns (~ret : int)
  requires (~int.gt(?a, 0) && ~int.gt(?b, 0));
  ensures ~eq(~ret, byteback.dummy.complete.GCD.gcd_recursive#int#int#(~heap, ?this, ?a, ?b));
{
  var $r : int;
  var $x : int;
  ...
  assert ~eq(byteback.dummy.complete.GCD.gcd_recursive#int#int#(~heap, $this, $r, $x), byteback.dummy.complete.GCD.gcd_recursive#int#int#(~heap, $this, $a, $b));
label4:
  assume (~int.gt($r, 0) && ~int.gt($x, 0));
  assume ~eq(byteback.dummy.complete.GCD.gcd_recursive#int#int#(~heap, $this, $r, $x), byteback.dummy.complete.GCD.gcd_recursive#int#int#(~heap, $this, $a, $b));
  if (($r == $x)) {
    goto label1;
  }
  if (($r <= $x)) {
