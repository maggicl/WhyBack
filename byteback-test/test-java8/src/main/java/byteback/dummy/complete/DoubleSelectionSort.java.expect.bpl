...
{
  var _this : Reference where (~typeof(~heap, _this) == $byteback.dummy.complete.DoubleSelectionSort);
  _this := ?this;
  call java.lang.Object.$init$##(_this);
  return;
}

function byteback.dummy.complete.DoubleSelectionSort.bounded_index#double?#int#(~heap : Store, _a : Reference, _i : int) returns (bool) { (~int.lte(0, _i) && ~int.lt(_i, ~lengthof(_a))) }

function byteback.dummy.complete.DoubleSelectionSort.is_minimum#double?#int#int#int#(~heap : Store, _a : Reference, _i : int, _j : int, _m : int) returns (bool) { (forall _$stack5 : int :: ~implies((~int.lte(_i, _$stack5) && ~int.lt(_$stack5, _j)), ~real.gte((~array.read(~heap, _a, _$stack5) : real), (~array.read(~heap, _a, _m) : real)))) }

function byteback.dummy.complete.DoubleSelectionSort.is_minimum#double?#int#int#(~heap : Store, _a : Reference, _i : int, _m : int) returns (bool) { byteback.dummy.complete.DoubleSelectionSort.is_minimum#double?#int#int#int#(~heap, _a, _i, ~lengthof(_a), _m) }
  ...
  ensures byteback.dummy.complete.DoubleSelectionSort.bounded_index#double?#int#(~heap, ?a, ~ret);
{
  ...
  assert byteback.dummy.complete.DoubleSelectionSort.is_minimum#double?#int#int#int#(~heap, _a, _i, _j, _m);
label3:
  assume (~int.lte(_i, _j) && ~int.lte(_j, ~lengthof(_a)));
  assume (~int.lte(_i, _m) && ~int.lt(_m, ~lengthof(_a)));
  assume byteback.dummy.complete.DoubleSelectionSort.is_minimum#double?#int#int#int#(~heap, _a, _i, _j, _m);
  if ((_j >= ~lengthof(_a))) {
    goto label1;
  }
  if ((~cmp((~array.read(~heap, _a, _j) : real), (~array.read(~heap, _a, _m) : real)) >= 0)) {
    goto label2;
  }
  _m := _j;
label2:
...
function byteback.dummy.complete.DoubleSelectionSort.sorted#double?#int#int#(~heap : Store, _a : Reference, _i : int, _j : int) returns (bool) { (forall _$stack4 : int :: ~implies((~int.lt(_i, _$stack4) && ~int.lt(_$stack4, _j)), ~real.lte((~array.read(~heap, _a, (_$stack4 - 1)) : real), (~array.read(~heap, _a, _$stack4) : real)))) }

function byteback.dummy.complete.DoubleSelectionSort.partitioned#double?#int#(~heap : Store, _a : Reference, _c : int) returns (bool) { (forall _$stack4 : int :: (forall _$stack5 : int :: ~implies((((~int.lte(0, _$stack4) && ~int.lt(_$stack4, _c)) && ~int.lte(_c, _$stack5)) && ~int.lt(_$stack5, ~lengthof(_a))), ~real.lte((~array.read(~heap, _a, _$stack4) : real), (~array.read(~heap, _a, _$stack5) : real))))) }
  ...
  modifies ~heap;
{
  ...
  assert byteback.dummy.complete.DoubleSelectionSort.partitioned#double?#int#(~heap, _a, _c);
  assert byteback.dummy.complete.DoubleSelectionSort.sorted#double?#int#int#(~heap, _a, 0, _c);
label2:
  assume (~int.lte(0, _c) && ~int.lte(_c, ~lengthof(_a)));
  assume byteback.dummy.complete.DoubleSelectionSort.partitioned#double?#int#(~heap, _a, _c);
  assume byteback.dummy.complete.DoubleSelectionSort.sorted#double?#int#int#(~heap, _a, 0, _c);
  if ((_c >= ~lengthof(_a))) {
    goto label1;
  }
  call ~sym1 := byteback.dummy.complete.DoubleSelectionSort.minimum#double?#int#(_a, _c);
  _$stack12 := ~sym1;
  _y := (~array.read(~heap, _a, _c) : real);
  ~heap := ~array.update(~heap, _a, _$stack12, (~array.read(~heap, _a, _c) : real));
