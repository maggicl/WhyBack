...
const unique $byteback.dummy.complete.DoubleSelectionSort : Type;

procedure byteback.dummy.complete.DoubleSelectionSort.$init$##(?this : Reference where (~typeof(~heap, ?this) == $byteback.dummy.complete.DoubleSelectionSort)) returns ()
{
  var $this : Reference where (~typeof(~heap, $this) == $byteback.dummy.complete.DoubleSelectionSort);
  $this := ?this;
  call java.lang.Object.$init$##($this);
  return;
}

function byteback.dummy.complete.DoubleSelectionSort.bounded_index#double?#int#(~heap : Store, $a : Reference, $i : int) returns (bool) { (~int.lte(0, $i) && ~int.lt($i, ~lengthof($a))) }

function byteback.dummy.complete.DoubleSelectionSort.is_minimum#double?#int#int#int#(~heap : Store, $a : Reference, $i : int, $j : int, $m : int) returns (bool) { (forall $$stack5 : int :: ~implies((~int.lte($i, $$stack5) && ~int.lt($$stack5, $j)), ~real.gte((~unbox(~heap.read(~heap, $a, ~element($$stack5))) : real), (~unbox(~heap.read(~heap, $a, ~element($m))) : real)))) }

function byteback.dummy.complete.DoubleSelectionSort.is_minimum#double?#int#int#(~heap : Store, $a : Reference, $i : int, $m : int) returns (bool) { byteback.dummy.complete.DoubleSelectionSort.is_minimum#double?#int#int#int#(~heap, $a, $i, ~lengthof($a), $m) }

procedure byteback.dummy.complete.DoubleSelectionSort.minimum#double?#int#(?a : Reference where (~typeof(~heap, ?a) == ~array.type(~Primitive)), ?i : int) returns (~ret : int)
  requires (~int.lte(0, ?i) && ~int.lt(?i, ~lengthof(?a)));
  ensures byteback.dummy.complete.DoubleSelectionSort.is_minimum#double?#int#int#int#(~heap, ?a, ?i, ~lengthof(?a), ~ret);
  ensures byteback.dummy.complete.DoubleSelectionSort.bounded_index#double?#int#(~heap, ?a, ~ret);
{
  var $m : int;
  var $j : int;
  ...
  assert byteback.dummy.complete.DoubleSelectionSort.is_minimum#double?#int#int#int#(~heap, $a, $i, $j, $m);
label3:
  assume (~int.lte($i, $j) && ~int.lte($j, ~lengthof($a)));
  assume (~int.lte($i, $m) && ~int.lt($m, ~lengthof($a)));
  assume byteback.dummy.complete.DoubleSelectionSort.is_minimum#double?#int#int#int#(~heap, $a, $i, $j, $m);
  if (($j >= ~lengthof($a))) {
    goto label1;
  }
  if ((~cmp((~unbox(~heap.read(~heap, $a, ~element($j))) : real), (~unbox(~heap.read(~heap, $a, ~element($m))) : real)) >= 0)) {
    goto label2;
  }
  $m := $j;
label2:
...
function byteback.dummy.complete.DoubleSelectionSort.sorted#double?#int#int#(~heap : Store, $a : Reference, $i : int, $j : int) returns (bool) { (forall $$stack4 : int :: ~implies((~int.lt($i, $$stack4) && ~int.lt($$stack4, $j)), ~real.lte((~unbox(~heap.read(~heap, $a, ~element(($$stack4 - 1)))) : real), (~unbox(~heap.read(~heap, $a, ~element($$stack4))) : real)))) }

function byteback.dummy.complete.DoubleSelectionSort.partitioned#double?#int#(~heap : Store, $a : Reference, $c : int) returns (bool) { (forall $$stack4 : int :: (forall $$stack5 : int :: ~implies((((~int.lte(0, $$stack4) && ~int.lt($$stack4, $c)) && ~int.lte($c, $$stack5)) && ~int.lt($$stack5, ~lengthof($a))), ~real.lte((~unbox(~heap.read(~heap, $a, ~element($$stack4))) : real), (~unbox(~heap.read(~heap, $a, ~element($$stack5))) : real))))) }

procedure byteback.dummy.complete.DoubleSelectionSort.sort#double?#(?a : Reference where (~typeof(~heap, ?a) == ~array.type(~Primitive))) returns ()
  requires ~neq(?a, ~null);
  requires ~int.gt(~lengthof(?a), 1);
  ensures byteback.dummy.complete.DoubleSelectionSort.sorted#double?#int#int#(~heap, ?a, 0, ~lengthof(?a));
  modifies ~heap;
{
  var $$stack12 : int;
  var $c : int;
  ...
  assert byteback.dummy.complete.DoubleSelectionSort.partitioned#double?#int#(~heap, $a, $c);
  assert byteback.dummy.complete.DoubleSelectionSort.sorted#double?#int#int#(~heap, $a, 0, $c);
label2:
  assume (~int.lte(0, $c) && ~int.lte($c, ~lengthof($a)));
  assume byteback.dummy.complete.DoubleSelectionSort.partitioned#double?#int#(~heap, $a, $c);
  assume byteback.dummy.complete.DoubleSelectionSort.sorted#double?#int#int#(~heap, $a, 0, $c);
  if (($c >= ~lengthof($a))) {
    goto label1;
  }
  call $$stack12 := byteback.dummy.complete.DoubleSelectionSort.minimum#double?#int#($a, $c);
  ~heap := ~heap.update(~heap, $a, ~element($$stack12), ~box((~unbox(~heap.read(~heap, $a, ~element($c))) : real)));
  ~heap := ~heap.update(~heap, $a, ~element($c), ~box((~unbox(~heap.read(~heap, $a, ~element($c))) : real)));
  $c := ($c + 1);
  assert (~int.lte(0, $c) && ~int.lte($c, ~lengthof($a)));
  assert byteback.dummy.complete.DoubleSelectionSort.partitioned#double?#int#(~heap, $a, $c);
  assert byteback.dummy.complete.DoubleSelectionSort.sorted#double?#int#int#(~heap, $a, 0, $c);
