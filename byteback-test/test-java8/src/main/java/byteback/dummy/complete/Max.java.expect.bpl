...
function byteback.dummy.complete.Max.max_in_range#int?#int#int#int#(~heap : Store, $a : Reference, $n : int, $start : int, $end : int) returns (bool) { (forall $$stack5 : int :: ~implies((~int.lte($start, $$stack5) && ~int.lt($$stack5, $end)), ~int.gte($n, (~unbox(~heap.read(~heap, $a, ~element($$stack5))) : int)))) }
