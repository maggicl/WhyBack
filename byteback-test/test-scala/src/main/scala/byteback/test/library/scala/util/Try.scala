...
package byteback.test.library.scala.util
...
final case class Failure[+T](@(Pure @getter) exception: Throwable) extends Try[T] {
  ...
  @Raise(exception = classOf[Throwable], when = "always")
